...
async fn main() {
    ...
    println!();
    println!("{}", "╔══════════════════════════════════════════════════════════════╗".cyan());
    println!("{}", "║    MWL Example 14: Human Agency Demo                        ║".cyan().bold());
    println!("{}", "╚══════════════════════════════════════════════════════════════╝".cyan());
    ...
    println!("  {} silence_is_consent():            {}", "├".dimmed(),
    ...
    println!("  {} emotional_signals_are_commitment(): {}", "├".dimmed(),
    ...
    println!("  {} validate_consent(false, None):   {}", "├".dimmed(),
    ...
    println!("  {} validate_consent(false, 24hrs):  {}", "├".dimmed(),
    ...
    println!("  {} validate_consent(true, None):    {}", "├".dimmed(),
    ...
    println!("  {}   success:        {}", "│".dimmed(),
        if disengage.success { "true".green() } else { "false".red() });
    println!("  {}   penalty_applied: {}", "└".dimmed(),
        if !disengage.penalty_applied { "false — no penalty for leaving".green() } else { "true — VIOLATION".red() });
    ...
    println!("  {} Normal interaction (20% attention, consented):", "├".dimmed());
    println!("  {}   exploitation detected: {}", "│".dimmed(),
    ...
    println!("  {} Coercive pattern (96% attention, no consent, escalating):", "├".dimmed());
    println!("  {}   exploitation detected: {}", "└".dimmed(),
    ...
    println!("  {} Allocated 40 to Agent A: {}", "├".dimmed(), "OK".green());

    budget.allocate(&agent_b, 35).unwrap();
    println!("  {} Allocated 35 to Agent B: {}", "├".dimmed(), "OK".green());

    budget.allocate(&agent_c, 25).unwrap();
    println!("  {} Allocated 25 to Agent C: {}", "├".dimmed(), "OK".green());

    println!("  {} Budget exhausted: {}", "├".dimmed(),
        if budget.is_exhausted() { "YES".yellow().bold() } else { "no".dimmed() });

    // Try to allocate more — should fail
    let overflow = budget.allocate(&wid(13), 1);
    println!("  {} Allocate 1 more unit: {}", "└".dimmed(),
        if overflow.is_err() {
    ...
    println!("  {}   Human:      max_initial_strength = {:.2}", "│".dimmed(), hp.coupling_limits.max_initial_strength);
    println!("  {}   Agent:      max_initial_strength = {:.2}", "│".dimmed(), ap.coupling_limits.max_initial_strength);
    println!("  {}   Financial:  max_initial_strength = {:.2}", "│".dimmed(), fp.coupling_limits.max_initial_strength);
    println!("  {}   World:      max_initial_strength = {:.2}", "│".dimmed(), wp.coupling_limits.max_initial_strength);

    // Merge Human + Agent
    let ha = merged_constraints(&hp, &ap);
    let min_ha = hp.coupling_limits.max_initial_strength
        .min(ap.coupling_limits.max_initial_strength);
    println!("  {} Human+Agent merged:", "├".dimmed());
    println!("  {}   max_initial_strength = {:.2} (min of {:.2}, {:.2})", "│".dimmed(),
        ha.coupling_limits.max_initial_strength,
        hp.coupling_limits.max_initial_strength,
        ap.coupling_limits.max_initial_strength);
    println!("  {}   respects max restriction: {}", "│".dimmed(),
    ...
    println!("  {}   merge(H,A) == merge(A,H): {}", "│".dimmed(),
        if (ha.coupling_limits.max_initial_strength - ah.coupling_limits.max_initial_strength).abs() < f64::EPSILON {
            "YES".green().bold()
        } else {
            "NO".red().bold()
        });
    ...
    println!("  {}   merge(H,H) == H: {}", "└".dimmed(),
        if (hh.coupling_limits.max_initial_strength - hp.coupling_limits.max_initial_strength).abs() < f64::EPSILON {
            "YES".green().bold()
        } else {
            "NO".red().bold()
        });
    ...
    println!("  {}   coercion_detection_enabled: {}", "│".dimmed(),
        if hp.human_involvement.coercion_detection_enabled { "true".green() } else { "false".red() });
    println!("  {}   require_human_for_high_risk: {}", "│".dimmed(),
        if hp.human_involvement.require_human_for_high_risk { "true".green() } else { "false".red() });
    println!("  {}   oversight_level: {:?}", "└".dimmed(), hp.human_involvement.oversight_level);

    // ── Summary ─────────────────────────────────────────────────────
    header("Summary");
    println!("  {} Consent Protocol:    {}", "├".dimmed(), "Silence never implies consent (I.S-1)".green());
    println!("  {} Coercion Detection:  {}", "├".dimmed(), "High-attention exploitation flagged (I.S-2)".green());
    println!("  {} Attention Budget:    {}", "├".dimmed(), "Finite resource, cannot be exhausted (I.S-BOUND)".green());
    println!("  {} Max Restriction:     {}", "├".dimmed(), "Most restrictive constraint wins (I.PROF-1)".green());
    println!("  {} Invariants:          {}", "└".dimmed(), "I.S-1, I.S-2, I.S-BOUND, I.PROF-1 demonstrated".green());
    println!();
}
