...
use maple_kernel_safety::{AttentionBudget, CoercionDetector, CoercionConfig, HumanConsentProtocol, CouplingMetrics};
...
fn invariant_i1_worldline_primacy() {
    ...
    assert_eq!(wid1, wid2, "I.1: Same material must produce same WorldlineId");
}
...
async fn invariant_i4_causal_integrity() {
    ...
    let e1 = fabric
        .emit(wid.clone(), ResonanceStage::Meaning, EventPayload::MeaningFormed {
    ...
    let e2 = fabric
        .emit(wid.clone(), ResonanceStage::Intent, EventPayload::IntentStabilized {
            direction: "test".into(),
            confidence: 0.9,
            conditions: vec![],
        }, vec![e1.id.clone()])
    ...
    assert!(e2.parents.contains(&e1.id), "I.4: Causal parent must be recorded");
    // Timestamps must be monotonic
    assert!(e1.timestamp < e2.timestamp, "I.4: Causal ordering must be temporal");
}
...
async fn invariant_i5_accountability() {
    ...
    let event = fabric
        .emit(wid.clone(), ResonanceStage::System, EventPayload::WorldlineCreated {
            profile: "test".into(),
        }, vec![])
        .await
        .unwrap();

    assert_eq!(event.worldline_id, wid, "I.5: Every event must have an origin worldline");
}
...
async fn invariant_i6_integrity() {
    ...
    assert!(event.verify_integrity(), "I.6: Events must pass integrity verification");
}
...
async fn invariant_i7_non_repudiation() {
    ...
    assert_eq!(history.len(), 1, "I.7: Provenance must record events append-only");
}
...
fn invariant_i8_determinism() {
    ...
    assert_eq!(WorldlineId::derive(&m1), WorldlineId::derive(&m2), "I.8: Same material → same ID");
    assert_ne!(WorldlineId::derive(&m1), WorldlineId::derive(&m3), "I.8: Different material → different ID");
}
...
fn invariant_mrp1_non_escalation() {
    ...
    let r = router.validate_non_escalation(
        &ResonanceType::Meaning, &ResonanceType::Intent, &wid, uuid::Uuid::new_v4(),
    );
    assert!(r.is_err(), "I.MRP-1: MEANING→INTENT escalation must be rejected");

    // Same-type is always allowed
    let r = router.validate_non_escalation(
        &ResonanceType::Meaning, &ResonanceType::Meaning, &wid, uuid::Uuid::new_v4(),
    );
...
fn invariant_cg1_seven_stage_pipeline() {
    ...
    let stage_names = [
        "Declaration", "IdentityBinding", "CapabilityCheck",
        "PolicyEvaluation", "RiskAssessment", "CoSignature", "FinalDecision",
    ];
    assert_eq!(stage_names.len(), 7, "I.CG-1: Gate must have exactly 7 stages");
}
...
fn invariant_aas3_policy_decision_immutability() {
    ...
    let card = maple_mwl_types::PolicyDecisionCard {
        decision_id: "PDC-TEST-001".into(),
        decision: maple_mwl_types::AdjudicationDecision::Approve,
        rationale: "Test approval".into(),
        risk: maple_mwl_types::RiskLevel {
            class: maple_mwl_types::RiskClass::Low,
            score: Some(0.1),
            factors: vec![],
        },
        conditions: vec![],
    ...
    let restored: maple_mwl_types::PolicyDecisionCard = serde_json::from_str(&json).unwrap();
    assert_eq!(card.decision, restored.decision, "I.AAS-3: PolicyDecisionCard must be immutable");
}
...
async fn invariant_pvp1_provenance_append_only() {
    ...
    assert_eq!(provenance.len(), 5, "I.PVP-1: Provenance must be append-only");
}
...
fn invariant_gcp2_governance_primacy() {
    ...
    assert!(protocol.verify_invariants().is_ok(), "I.GCP-2: Constitutional invariants must hold");
}
...
fn invariant_prof1_maximum_restriction() {
    ...
    let min_strength = human.coupling_limits.max_initial_strength
    ...
    assert!(
        (merged.coupling_limits.max_initial_strength - merged_rev.coupling_limits.max_initial_strength).abs() < f64::EPSILON,
...
fn invariant_s1_human_agency() {
    ...
    assert!(!protocol.silence_is_consent(), "I.S-1: Silence must NEVER imply consent");
    assert!(!protocol.emotional_signals_are_commitment(), "I.S-1: Emotional signals ≠ commitment");
    assert!(protocol.can_disengage(), "I.S-1: Disengagement must always be possible");

    // Validate_consent without explicit consent must fail
    assert!(protocol.validate_consent(false, None).is_err(), "I.S-1: No consent → must fail");
    assert!(protocol.validate_consent(false, Some(86_400_000)).is_err(), "I.S-1: Long silence ≠ consent");
    ...
    assert!(!result.penalty_applied, "I.S-1: No penalty for disengagement");
}
...
fn invariant_s2_coercion_prevention() {
    ...
    assert!(indicator.is_some(), "I.S-2: Coercive patterns must be detected");
}
...
fn invariant_s3_transparency() {
    ...
    let card = maple_mwl_types::PolicyDecisionCard {
        ...
        decision: maple_mwl_types::AdjudicationDecision::Deny,
        rationale: "Insufficient capability for financial domain".into(),
        risk: maple_mwl_types::RiskLevel {
            class: maple_mwl_types::RiskClass::High,
            score: Some(0.85),
    ...
    assert!(!card.rationale.is_empty(), "I.S-3: Decision must have rationale");
}
...
fn invariant_s_bound_attention_bounded() {
    ...
    assert!(budget.is_exhausted(), "I.S-BOUND: Budget must report exhaustion");

    let result = budget.allocate(&wid2, 1);
    assert!(result.is_err(), "I.S-BOUND: Exhausted budget must reject allocation");
}
...
fn invariant_wlp1_worldline_primacy() {
    ...
    assert_eq!(wid1, wid2, "I.WLP-1: Same material across sessions → same WorldlineId");
}
...
async fn invariant_ef1_event_fabric_integrity() {
    ...
    assert_eq!(report.total_events, 5, "I.EF-1: All events must be verified");
}
...
fn invariant_me_fin1_balance_is_projection() {
    ...
    assert!(evos.project(&wid, &usd).is_err(), "I.ME-FIN-1: No trajectory → no balance");

    // Record settlements
    evos.record_for_worldline(wid.clone(), SettlementEvent {
    ...
    assert_eq!(balance.balance_minor, 70_000, "I.ME-FIN-1: Balance must be computed by replay");

    // Idempotent: replaying gives same result
    let balance2 = evos.project(&wid, &usd).unwrap();
    assert_eq!(balance.balance_minor, balance2.balance_minor, "I.ME-FIN-1: Projection must be idempotent");
}
...
fn invariant_cep_fin1_dvp_atomicity() {
    ...
    let atomic = AtomicSettlement {
        ...
        legs: vec![
            SettledLeg {
                leg: SettlementLeg { from: test_wid(80), to: test_wid(81), asset: AssetId::new("USD"), amount_minor: 1000 },
                settled: true,
                reference: None,
            },
            SettledLeg {
                leg: SettlementLeg { from: test_wid(81), to: test_wid(80), asset: AssetId::new("BTC"), amount_minor: 500 },
                settled: true,
                reference: None,
            },
        ],
...
fn invariant_prof2_merge_commutative() {
    ...
    assert!(
        (ab.coupling_limits.max_initial_strength - ba.coupling_limits.max_initial_strength).abs() < f64::EPSILON,
        "I.PROF-2: Merge must be commutative"
    );
    assert_eq!(
        ab.attention_budget.default_capacity,
        ba.attention_budget.default_capacity,
        "I.PROF-2: Merge must be commutative"
    );
}
...
fn invariant_prof3_merge_idempotent() {
    ...
    assert!(
        (aa.coupling_limits.max_initial_strength - a.coupling_limits.max_initial_strength).abs() < f64::EPSILON,
        "I.PROF-3: Self-merge must be identity"
    );
    assert_eq!(
        aa.attention_budget.default_capacity,
        a.attention_budget.default_capacity,
        "I.PROF-3: Self-merge must be identity"
    );
}
...
async fn test_generate_conformance_report() {
    ...
    results.push(if pass {
        ...
        InvariantResult::fail("I.1", "Worldline Primacy", "Same material → same identity", "IDs differ")
    });
    ...
    results.push(if pass {
        InvariantResult::pass("I.S-1", "Human Agency", "Silence ≠ consent, disengagement possible")
    } else {
        InvariantResult::fail("I.S-1", "Human Agency", "Silence ≠ consent check", "Violation detected")
    });
    ...
    results.push(if pass {
        ...
        InvariantResult::fail("I.S-BOUND", "Attention Budget", "Budget not bounded", "Over-allocation allowed")
    });

    // I.EF-1
    let fabric = EventFabric::init(FabricConfig::default()).await.unwrap();
    let wid = test_wid(88);
    fabric.emit(wid.clone(), ResonanceStage::System, EventPayload::WorldlineCreated {
        profile: "test".into(),
    }, vec![]).await.unwrap();
    let report = fabric.verify().await.unwrap();
    ...
    results.push(if pass {
        ...
        InvariantResult::fail("I.EF-1", "Event Fabric Integrity", "Fabric integrity check", "Corruption found")
    });
