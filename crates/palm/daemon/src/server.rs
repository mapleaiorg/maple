...
use maple_storage::postgres::PostgresMapleStorage;
...
impl Server {
    ...
    pub async fn new(config: DaemonConfig) -> DaemonResult<Self> {
        ...
        let agent_kernel_storage: Arc<dyn MapleStorage> = match &config.storage {
            StorageConfig::Memory => Arc::new(InMemoryMapleStorage::new()),
            StorageConfig::Postgres {
                url,
                max_connections,
                connect_timeout_secs,
            } => match PostgresMapleStorage::connect_with_options(
                url,
                *max_connections,
                *connect_timeout_secs,
            )
            .await
            {
                ...
                Err(err) if Self::should_fallback_to_memory(&config.platform) => {
                    tracing::warn!(
                        "AgentKernel PostgreSQL storage init failed in Development profile: {}. Falling back to in-memory AgentKernel storage.",
                        err
                    );
                    Arc::new(InMemoryMapleStorage::new())
                }
                Err(err) => {
                    return Err(DaemonError::Config(format!(
                        "AgentKernel storage initialization failed: {}",
                        err
                    )))
                }
            },
        };
        let agent_kernel = Arc::new(AgentKernel::new_with_storage(runtime, agent_kernel_storage));
