//! Grok provider adapter for MAPLE cognition.
//!
...
pub const DEFAULT_MODEL: &str = "grok-2";
pub const AUTH_ENV_VAR: &str = "XAI_API_KEY";

/// Minimal transport request type for Grok completions.
...
/// Minimal transport response type for Grok completions.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub trait GrokTransport: Send + Sync {
    async fn complete(
        ...
        request: &GrokRequest,
        api_key: &str,
    ) -> Result<GrokResponse, ModelAdapterError>;
}
...
#[async_trait]
impl GrokTransport for NoopTransport {
    async fn complete(
        &self,
        _request: &GrokRequest,
        _api_key: &str,
    ) -> Result<GrokResponse, ModelAdapterError> {
        Err(ModelAdapterError::new(
            ModelBackend::Grok,
            ModelErrorKind::Transport,
            "Grok transport not implemented",
        ))
    }
}
...
#[derive(Clone)]
pub struct GrokModelAdapter {
    config: ModelProviderConfig,
    transport: Arc<dyn GrokTransport>,
}

impl std::fmt::Debug for GrokModelAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GrokModelAdapter")
            .field("config", &self.config)
            .finish()
    }
}

impl GrokModelAdapter {
    pub fn new(model: impl Into<String>) -> Self {
        Self::with_transport(model, Arc::new(NoopTransport))
    }

    pub fn with_transport(model: impl Into<String>, transport: Arc<dyn GrokTransport>) -> Self {
        Self {
    ...
    fn fallback_response(&self, prompt: &str) -> ModelResponse {
        ModelResponse {
            ...
            raw_text: format!("grok:fallback:{}", prompt),
            cognition: StructuredCognition {
                meaning_summary: "Grok adapter requires explicit structured transport output"
                    .to_string(),
...
#[async_trait]
impl ModelAdapter for GrokModelAdapter {
    ...
    async fn infer(&self, request: &ModelRequest) -> Result<ModelResponse, ModelAdapterError> {
        ...
        if let Some(raw) = &request.raw_response_override {
            return Ok(ModelResponse {
                ...
                raw_text: raw.clone(),
                cognition: StructuredCognition {
                    meaning_summary: "Grok override response requires higher-level parser"
                        .to_string(),
                    intent: "request_clarification".to_string(),
                    confidence: 0.0,
                    suggested_tool: None,
    ...
    async fn propose_intent(
        ...
        Ok(IntentDraft {
            objective: format!("grok_intent_stub:{}", meaning.summary.trim()),
            steps: vec![
    ...
    async fn summarize(
        ...
        let summary = if journal_slice.is_empty() {
            "Grok summary: no events".to_string()
        } else {
            format!("Grok summary: {} events processed", journal_slice.len())
        };
...
mod tests {
    ...
    async fn stable_intent_stub() {
        let adapter = GrokModelAdapter::default_stub();
        let intent = adapter
            .propose_intent(
        ...
        assert!(intent.objective.starts_with("grok_intent_stub:"));
