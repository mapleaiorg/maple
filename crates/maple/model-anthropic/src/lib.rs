//! Anthropic provider adapter for MAPLE cognition.
//!
...
pub const AUTH_ENV_VAR: &str = "ANTHROPIC_API_KEY";

/// Minimal transport request type for Anthropic completions.
...
/// Minimal transport response type for Anthropic completions.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub trait AnthropicTransport: Send + Sync {
    async fn complete(
        ...
        api_key: &str,
    ) -> Result<AnthropicResponse, ModelAdapterError>;
}
...
impl AnthropicTransport for NoopTransport {
    async fn complete(
        ...
        Err(ModelAdapterError::new(
            ...
            "Anthropic transport not implemented",
        ))
    }
}
...
impl std::fmt::Debug for AnthropicModelAdapter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AnthropicModelAdapter")
...
impl AnthropicModelAdapter {
    ...
    fn fallback_response(&self, prompt: &str) -> ModelResponse {
        ModelResponse {
            ...
            raw_text: format!("anthropic:fallback:{}", prompt),
            cognition: StructuredCognition {
                meaning_summary: "Anthropic adapter requires explicit structured transport output"
                    .to_string(),
...
impl ModelAdapter for AnthropicModelAdapter {
    ...
    async fn infer(&self, request: &ModelRequest) -> Result<ModelResponse, ModelAdapterError> {
        ...
        if let Some(raw) = &request.raw_response_override {
            return Ok(ModelResponse {
                ...
                raw_text: raw.clone(),
                cognition: StructuredCognition {
                    meaning_summary: "Anthropic override response requires higher-level parser"
                        .to_string(),
                    intent: "request_clarification".to_string(),
                    confidence: 0.0,
                    suggested_tool: None,
    ...
    async fn propose_intent(
        ...
        Ok(IntentDraft {
            objective: format!("anthropic_intent_stub:{}", meaning.summary.trim()),
            steps: vec![
    ...
    async fn summarize(
        ...
        let summary = if journal_slice.is_empty() {
            "Anthropic summary: no events".to_string()
        } else {
            format!(
                "Anthropic summary: {} events processed",
                journal_slice.len()
            )
        };
...
mod tests {
    ...
    async fn stable_intent_stub() {
        let adapter = AnthropicModelAdapter::default_stub();
        let intent = adapter
            .propose_intent(
        ...
        assert!(intent.objective.starts_with("anthropic_intent_stub:"));
